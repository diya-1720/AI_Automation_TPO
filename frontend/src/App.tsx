...
import { FilePlus, Folder, FileText, Settings as SettingsIcon, LogOut, Users, Shield } from 'lucide-react';
...
function Sidebar({ user, onLogout }: { user: any; onLogout: () => void }) {
  ...
  return (
    <div className="w-64 bg-slate-900 text-slate-100 min-h-screen flex flex-col justify-between border-r border-slate-800 flex-shrink-0">
      <div>
...
function AppContent({
  ...
  return (
    <div className={`flex min-h-screen ${theme === 'dark' ? 'dark bg-slate-900 text-slate-100' : 'bg-gray-50 text-gray-900'}`}>
